...
class _ParkingSlotState extends State<ParkingSlot> {
  ...
  Future<void> _checkMyBooking() async {
    ...
    bool found = history.any((b) => b.contains(widget.locationName) && b.contains('№${widget.id}'));
    if (mounted) setState(() { isBookedByMe = found; });
  }

  void _bookSlot() async {
    await LocalAuthRepository().addBookingToHistory(widget.locationName, widget.id);
    _checkMyBooking();
    if (mounted) {
    ...
    return GestureDetector(
      onTap: () {
        if (!isPermanentlyOccupied) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Це місце вже зайняте'), behavior: SnackBarBehavior.floating)
          );
        }
      },
      child: Container(
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(widget.locationName, style: const TextStyle(color: Colors.white54, fontSize: 14)),
            const Divider(color: Colors.white10, height: 30),
            
            _infoRow(Icons.info_outline, 'Статус: ${free ? "Вільно" : "Заброньовано вами"}'),
            ...
            if (free) 
              ElevatedButton(
                ...
                onPressed: _bookSlot,
                child: const Text('ЗАБРОНЮВАТИ', style: TextStyle(fontWeight: FontWeight.bold)),
              ),
