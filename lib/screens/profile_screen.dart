...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void _showLogoutDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Ви впевнені, що хочете вийти з профілю?',
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Center(
                        child: Text(_user?.fullName ?? 'Maria',
                            style: const TextStyle(
                                fontSize: 22, fontWeight: FontWeight.bold))),
                    Center(
                        child: Text(_user?.email ?? 'tsiupka2305@gmail.com',
                    ...
                    GlassCard(
                      child: ListTile(
                        leading: const Icon(Icons.directions_car,
                            color: Colors.white),
                        title: Text(
                            '${_user?.carBrand ?? "Tesla"} ${_user?.carModel ?? "Model 3"}'),
                    ...
                    FutureBuilder<List<String>>(
                      ...
                      builder: (context, snapshot) {
                        if (!snapshot.hasData || snapshot.data!.isEmpty) {
                          return const Text('Історія порожня',
                        ...
                        return Column(
                          children: snapshot.data!.asMap().entries.map((entry) {
                            ...
                            int slotNum = int.tryParse(parts[1]
                                    .replaceAll(RegExp(r'[^0-9]'), '')) ??
                                0;
                            return _detailedHistoryItem(parts[0], parts[1],
                                parts[2], "25 грн/год", slotNum % 2 == 0, idx);
                          }).toList(),
                        );
                      },
                    ),
                    const SizedBox(height: 30),
                    const Text('ОСТАННІ СЕСІЇ',
                        style:
                            TextStyle(color: Colors.cyanAccent, fontSize: 12)),
                    const SizedBox(height: 10),
                    _sessionItem('ТЦ Форум', '2 год 15 хв', '45 UAH',
                        Colors.greenAccent),
                    _sessionItem(
                        'Паркінг Валова', '45 хв', '15 UAH', Colors.white70),
                  ],
                ),
              ),
            ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  OutlinedButton.icon(
                    ...
                    label: const Text('ВИДАЛИТИ АККАУНТ',
  ...
  Widget _detailedHistoryItem(String loc, String slot, String time,
      String price, bool hasBolt, int index) {
    return Container(
  ...
  Widget _sessionItem(
          String loc, String duration, String price, Color priceColor) =>
      Padding(
