...
class MqttService {
    ...
    try {
      ...
      if (client!.connectionStatus?.state == MqttConnectionState.connected) {
        client!.updates?.listen((List<MqttReceivedMessage<MqttMessage>> messages) {
          ...
          String officialName = "";
          if (topic.contains("forum")) officialName = "ТЦ Форум Львів";
          else if (topic.contains("victoria")) officialName = "ТРЦ Victoria Gardens";
          else if (topic.contains("airport")) officialName = "Аеропорт Львів";
          else if (topic.contains("valova")) officialName = "Паркінг на Валовій";

          if (officialName.isNotEmpty) {
            _messageController.add({"name": officialName, "value": payload});
          }
        });
        return true;
      }
  ...
  void subscribe(String topic) => client?.subscribe(topic, MqttQos.atMostOnce);
